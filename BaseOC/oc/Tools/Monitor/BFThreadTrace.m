...
static inline dispatch_queue_t log_IO_queue(void) {
    static dispatch_queue_t log_IO_queue;
    static dispatch_once_t once;
    dispatch_once(&once, ^{
        log_IO_queue = dispatch_queue_create("com.sindrilin.log_IO_queue", NULL);
    });
    return log_IO_queue;
}

@implementation BFThreadTrace
...
+ (NSString *)backtraceOfAllThread {
    ...
    for (int idx = 0; idx < thread_count; idx++) {
        [result appendString:backtraceOfThread(threads[idx])];
    }
    return result.copy;
}
...
+ (NSString *)backtraceOfNSThread:(NSThread *)thread {
    return backtraceOfThread(machThreadFromNSThread(thread));
}

+ (void)logMain {
    LOG_SEPERATE
    NSLog(@"%@", [self backtraceOfMainThread]);
    LOG_SEPERATE
}

+ (void)logCurrent {
    LOG_SEPERATE
    NSLog(@"%@", [self backtraceOfCurrentThread]);
    LOG_SEPERATE
}

+ (void)logAllThread {
    LOG_SEPERATE
    NSLog(@"%@", [self backtraceOfAllThread]);
    LOG_SEPERATE
}
...
void symbolicate(const uintptr_t * const backtraceBuffer, Dl_info * const symbolsBuffer, const int numEntries, const int skippedEntries) {
    int idx = 0;
    if (!skippedEntries && idx < numEntries) {
        dladdress(backtraceBuffer[idx], &symbolsBuffer[idx]);
        idx++;
    }
    
    for (; idx < numEntries; idx++) {
        dladdress(CALL_INSTRUCTION_FROM_RETURN_ADDRESS(backtraceBuffer[idx]), &symbolsBuffer[idx]);
    }
}

bool dladdress(const uintptr_t address, Dl_info * const info) {
    info->dli_fname = NULL;
    info->dli_fbase = NULL;
    info->dli_sname = NULL;
    info->dli_saddr = NULL;
...
+ (NSString *)backtraceLogFilePath {
    static NSString * const fileDirectoryName = @"backtrace";
...
+ (void)recordLoggerWithFileName: (NSString *)fileName {
    ...
    dispatch_async(log_IO_queue(), ^{
        ...
        NSString * backtraceStackInfo = [self backtraceOfMainThread];
