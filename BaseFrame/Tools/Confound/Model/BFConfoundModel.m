...
@implementation BFConfoundModel

+ (NSArray *)data{
    BFConfoundSetting *set = BFConfoundSetting.sharedManager;
    NSArray *data = @[
    @{@"idStr":@"1",@"title":@"添加垃圾代码",@"setting":@1,@"selecte":@(set.isSpam),@"url":BFString.vc_spam_code},
    @{@"idStr":@"2",@"title":@"修改工程名(`Podfile`被修改后需要`pod install`)",@"setting":@1,@"selecte":@(set.isModifyProject),@"url":BFString.vc_modify_project},
    @{@"idStr":@"3",@"title":@"修改文件名前缀(包含类名之外)",@"setting":@1,@"selecte":@(set.isSpam),@"url":BFString.vc_modify_class},
    @{@"idStr":@"4",@"title":@"删除注释",@"setting":@0,@"selecte":@(set.isClearComment)},
    @{@"idStr":@"5",@"title":@"方法名混淆",@"setting":@0,@"selecte":@(set.isMethodConfusion)}
    ];
    return [NSArray yy_modelArrayWithClass:[BFConfoundModel class] json:data];
}

+ (NSArray *)data_file{
    BFSpamCodeFileSetting *fileSet = BFConfoundSetting.sharedManager.spamSet.spamFileSet;
...
+ (NSArray *)data_code{
    ...
    NSArray *data = @[@{@"idStr":@"11",@"title":@"在原文件中添加垃圾方法",@"setting":@0,@"selecte":@(codeSet.isSpamInOldCode)},
                      @{@"idStr":@"12",@"title":spamCodeDirName,@"setting":@1,@"selecte":@(codeSet.isSpamInNewDir),@"url":BFString.vc_spam_code_model},
                      @{@"idStr":@"13",@"title":@"新增方法名配置",@"setting":@1,@"selecte":@(codeSet.isSpamMethod),@"url":BFString.vc_spam_code_method},
                      @{@"idStr":@"14",@"title":@"取项目中单词来命名'类名|方法名'",@"setting":@1,@"selecte":@(codeSet.isSpamOldWords),@"url":BFString.vc_spam_code_word}];
    return [NSArray yy_modelArrayWithClass:[BFConfoundModel class] json:data];
}

+ (NSArray *)data_code_method{
    BFSpamCodeSetting *codeSet = BFConfoundSetting.sharedManager.spamSet;
...
+ (NSArray *)data_code_word{
    BFSpamCodeSetting *set = BFConfoundSetting.sharedManager.spamSet;
    BFSpamCodeWordSetting *wordSet = set.spamWordSet;
...
+ (NSArray *)data_modify_project{
    BFModifyProjectSetting *modifySet = BFConfoundSetting.sharedManager.modifySet;
...
+ (void)editContent:(id)content idStr:(NSString *)idStr{
    ...
    BFSpamCodeSetting *codeSet = set.spamSet;
    BFSpamCodeFileSetting *fileSet = codeSet.spamFileSet;
    BFSpamCodeWordSetting *wordSet = codeSet.spamWordSet;
    BFModifyProjectSetting *modify = set.modifySet;
